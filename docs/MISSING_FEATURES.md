...
| Find / Replace | Partial | See Dialogs section. Searches any document with a text buffer — code windows and the carried-file editor alike — under either of two scopes; a third, project-wide, is not offered ([#372](https://github.com/hexide-io/HexIDE/issues/372)) |
...
| Find scope (project / all docs) | Partial | **All Open Documents** works — Find and Replace All both cross every open document with a text buffer, and a match in another one brings that tab to the front. **Current Project** is not offered at all: the combo entry was removed rather than left decorative, because a module that is not open has only its last-*saved* text in `ModuleDefinition.Code`, so searching it would report hits at offsets that no longer exist ([#372](https://github.com/hexide-io/HexIDE/issues/372)) |
...
| Add Procedure | Done | |
| Find / Replace | Partial | Searches any document that has a text buffer, not only VB6 code windows — the carried-file editor included. Both offered scopes are honoured (Current Module, All Open Documents); project-wide is deferred to [#372](https://github.com/hexide-io/HexIDE/issues/372). Edit ▸ Find / Replace / F3 grey out on a document with nothing to search (the designer, the Object Browser, the Translation Editor, Language & Debug Servers), and the modeless dialog says so if the active tab changes under it |

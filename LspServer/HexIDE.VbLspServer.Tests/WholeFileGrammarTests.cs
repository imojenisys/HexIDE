...
public class WholeFileGrammarTests
{
    ...
    public void WholeFilesParseThroughTheServerGrammar()
    {
        ...
        newRepoFailures.Should().BeEmpty(
            "a file in this repository whose body this grammar cannot parse is a grammar gap — add it to "
            + nameof(KnownBodyFailures) + " with an issue number once it is filed, or fix it.\n"
            + string.Join("\n", newRepoFailures));
    }

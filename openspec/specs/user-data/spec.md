...
## Purpose
Where HexIDE keeps what belongs to one user rather than to a project — settings, window layout, recent
projects, language server configuration, add-in consent and revocations, translations — and how a session is
pointed somewhere else. The contract is that one session has exactly one such directory, because every way of
getting that wrong fails silently: a setting that does not persist, consent asked twice, a revocation that is
never found.
